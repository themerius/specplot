...
def _get_version() -> str:
    """Get the specplot version from package metadata or pyproject.toml."""
    # Try installed package metadata first
    try:
        from importlib.metadata import version
        return version("specplot")
    except Exception:
        pass

    # Fall back to reading pyproject.toml (dev mode)
    try:
        import tomllib
...
def render_to_svg(diagram: Diagram, filename: str | None = None) -> str:
    ...
    # Add header comment with version
    version = _get_version()
    header = f"<!-- Generated by specplot v{version} - https://github.com/themerius/specplot -->\n"

    # Insert after XML declaration if present, otherwise prepend
    if svg_content.startswith('<?xml'):
        end_decl = svg_content.find('?>') + 2
        svg_content = svg_content[:end_decl] + '\n' + header + svg_content[end_decl:].lstrip()
    else:
